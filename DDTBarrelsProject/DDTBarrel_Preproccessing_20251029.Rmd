...
taxonomy <- read_qza(paste0(path,"05.1-taxonomy-blast-90-1-hannahmeiodb-20251110.qza")) #Read tax table
...
#Adjust the halo column to accurately reflect what I think #Bacterial_Halo
for(i in c(1:nrow(metadata))){
  if(metadata$Unique_Barrel[i] %in% c("Barrel 3", "Barrel 4", "Barrel 5")){
  metadata$Bacterial_Halo[i] = "Yes"
  } else {
  metadata$Bacterial_Halo[i] = "No"
  } }

metadata$On_Halo <- NA #Zone B of halo barrels
for(i in c(1:nrow(metadata))){
  if(metadata$Bacterial_Halo[i] == "Yes" & metadata$Zone[i] == "B"){
  metadata$On_Halo[i] = "Yes"
  } else {
  metadata$On_Halo[i] = "No"
  } }
```

...
plot(sample_sums(ddt_phylo)) #for cabight and ddt_phylo
hist(sample_sums(ddt_merged2))
plot_bar(ddt_merged2) #do before and after merge
plot_bar(ddt_norm)
...
#spec_ca <- specaccum(otu_table(cabight_phylo),  method = "random", permutations = 100)
#takes >1hr to run, having difficulties with understanding?

## Pulled this section of SAC code from the Cordier github fig 1c ##
#want to make curves for DDT background and DDT barrels -- each habitat
ddt_barrels <- subset_samples(ddt_phylo, habitat == "DDT_Barrel")
ddt_background <- subset_samples(ddt_phylo, habitat == "DDT_Background")
ddt_controls <- subset_samples(ddt_phylo, habitat != "DDT_Barrel" & habitat != "DDT_Background" )
## specaccum 
sp_accum_ddt_barrel <- specaccum(otu_table(ddt_barrels), method = "random")
sp_accum_ddt_bg <- specaccum(otu_table(ddt_background), method = "random")
sp_accum_ddt_control <- specaccum(otu_table(ddt_controls), method = "random")
## Plot accumulation curves
pdf("ASVs_accumulation_curves_sampling_effort_habitats.pdf", width=3.5, height=4)
plot(sp_accum_ddt_barrel, col = "cadetblue1", ylab = "#ASVs (10³)", xlab = "#Samples", ci.type="poly", lwd=2, ylim= c(1,length(otu_table(ddt_barrels))+1000), ci.lty=0, ci.col=alpha("cadetblue1", alpha = 0.2), cex.axis=0.6)

lines(sp_accum_ddt_bg, col = "darkgoldenrod1", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("darkgoldenrod1", alpha = 0.2))
lines(sp_accum_ddt_control, col = "blue4", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("blue4", alpha = 0.2))
dev.off()
```
...
plot_richness(ddt_merged2, x = "habitat", measures = c("Shannon", "Observed")) +
  geom_boxplot(aes(fill = site)) 
plot_richness(ddt_merged2, x = "barrel_zone", measures = c("Shannon", "Observed")) +
  geom_boxplot(aes(fill = core_fraction)) #~habitat, core_fraction

#NMDS
set.seed(1234)
ord <- ordinate(ddt_norm, "NMDS", "bray",k=3,permutations=999)
plot_ordination(ddt_norm,ord,type="samples",color="core_fraction")+
  theme(axis.title = element_text(size = 10, face = "bold", colour = "grey30"), 
        panel.background = element_blank(), panel.border = element_rect(fill = NA, colour = "grey30"), 
        axis.ticks = element_blank(), axis.text = element_blank(), legend.key = element_blank(), 
        legend.title = element_text(size = 10, face = "bold", colour = "grey30"), 
        legend.text = element_text(size = 9, colour = "grey30")) #+
facet_grid(~barrel_zone)
...
###Alpha Diversity Analysis - come back later
```{r}

plot_richness(ddt_metazoans, x = "habitat", measures = c("Observed")) +
  geom_boxplot(aes(fill = site)) #+ facet_wrap(habitat ~ ., scales = "free", drop = TRUE, ncol = 2, shrink = T) +
  theme_bw() #+
  theme(axis.text.x = element_text(angle = 90))#from github repo

plot_richness(ddt_phylo_filt, x = "habitat", measures = c("Shannon")) +
  geom_boxplot(aes(fill = barrel_ID))+ #from github repo
  facet_grid(~B18_stratum, scales = "free")

plot_richness(ddt_phylo_filt, x = "barrel_zone", measures = c("Shannon")) +
  geom_boxplot(aes(fill = barrel_ID))#+ #from github repo
  facet_grid(~B18_stratum, scales = "free")

plot_richness(ddt_phylo_megafilt, x = "barrel_zone", measures = c("Shannon")) +
  geom_boxplot(aes(fill = barrel_ID))+ #from github repo
  facet_grid(~core_fraction, scales = "free") 

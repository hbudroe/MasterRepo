...
#do spec by various sample types
mzg_epi <- subset_samples(mzg_raw, DepthZone == "Epipelagic")
mzg_meso <- subset_samples(mzg_raw, DepthZone == "Mesopelagic")
sp_accum_mzg_epi <- specaccum(otu_table(mzg_epi), method = "random")
sp_accum_mzg_meso <- specaccum(otu_table(mzg_meso), method = "random")
...
#do spec by various sample types ~ salp nsalp
mzg_salp <- subset_samples(mzg_raw, SalpType == "Salp")
mzg_no <- subset_samples(mzg_raw, SalpType == "Non-Salp")
sp_accum_mzg_salp <- specaccum(otu_table(mzg_salp), method = "random")
sp_accum_mzg_no <- specaccum(otu_table(mzg_no), method = "random")
#plot the accumulation curves
pdf("specaccum", width=3.5, height=4) #default from code
plot(sp_accum_mzg_salp, col = "cadetblue1", ylab = "#ASVs (10³)", 
     xlab = "#Samples", ci.type="poly", lwd=2, ylim= c(1,10000), 
     ci.lty=0, ci.col=alpha("cadetblue1", alpha = 0.2), cex.axis=0.6) #have to change data name + ylim
lines(sp_accum_mzg_no, col = "darkgoldenrod1", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("darkgoldenrod1", alpha = 0.2))
#lines(sp_accum_gp_skin, col = "blue4", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("blue4", alpha = 0.2))
dev.off()

#do spec by various sample types ~ cycle
mzg_1 <- subset_samples(mzg_raw, ID == "SA1")
mzg_2 <- subset_samples(mzg_raw, ID == "SA2")
mzg_5 <- subset_samples(mzg_raw, ID == "SA3")
mzg_3 <- subset_samples(mzg_raw, ID == "ST1")
mzg_4 <- subset_samples(mzg_raw, ID == "ST2")
sp_accum_mzg_1 <- specaccum(otu_table(mzg_1), method = "random")
sp_accum_mzg_2 <- specaccum(otu_table(mzg_2), method = "random")
sp_accum_mzg_5 <- specaccum(otu_table(mzg_5), method = "random")
sp_accum_mzg_3 <- specaccum(otu_table(mzg_3), method = "random")
sp_accum_mzg_4 <- specaccum(otu_table(mzg_4), method = "random")
#plot the accumulation curves
pdf("specaccum", width=3.5, height=4) #default from code
plot(sp_accum_mzg_1, col = "red", ylab = "#ASVs (10³)", 
     xlab = "#Samples", ci.type="poly", lwd=2, ylim= c(1,10000), 
     ci.lty=0, ci.col=alpha("red", alpha = 0.2), cex.axis=0.6) #have to change data name + ylim
lines(sp_accum_mzg_2, col = "orange", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("orange", alpha = 0.2))
lines(sp_accum_mzg_5, col = "darkgoldenrod1", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("darkgoldenrod1", alpha = 0.2))
lines(sp_accum_mzg_3, col = "green4", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("green4", alpha = 0.2))
lines(sp_accum_mzg_4, col = "blue4", ci.type="poly", lwd=2, ci.lty=0, ci.col=alpha("blue4", alpha = 0.2))
dev.off()
```
...
mzg_raw %>% subset_taxa(Phylum!="Echinodermata") %>% subset_taxa(Phylum !="Bryozoa") %>% 
  ...
  subset_taxa(Phylum !="Platyhelminthes") %>% subset_taxa(Phylum !="Rotifera") %>% subset_taxa(Order!="Scleractinia") %>% filter_taxa(function(x) sum(x > 0) > 1, TRUE) -> mzg_filt_unnorm #764 #9156882 reads summed
...
mzg_raw %>% subset_taxa(Phylum!="Echinodermata") %>% subset_taxa(Phylum !="Bryozoa") %>% 
  subset_taxa(Phylum !="Sipuncula") %>% subset_taxa(Phylum !="Nemertea") %>%
  subset_taxa(Phylum !="Platyhelminthes") %>% subset_taxa(Phylum !="Rotifera") %>% subset_taxa(Order!="Scleractinia") %>% filter_taxa(function(x) sum(x > 0) > 1, TRUE) -> deseqps #764 #9156882 reads summed


otu_table(deseqps)=otu_table(deseqps)+1
diagdds = phyloseq_to_deseq2(deseqps, ~ WaterType)
diagdds = DESeq(diagdds, test="Wald", fitType="mean") #fit type parametric didn't capture dispersion trend -> told to do local or mean
